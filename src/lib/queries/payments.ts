...
export async function refundPartial(input: RefundPartialInput): Promise<RefundPartialResult> {
  ...
  // Single endpoint: Stripe's /refunds works for cards, Klarna, and
  // Clearpay alike. Klarna+Terminal payments are Stripe-managed
  // PIs, so refunding follows the same path as a card refund.
