...
4. **Voice call mode is a view preference, not a permission.** It lives in a small React context, is available to flagged agents and to admins (who never count as capacity), defaults on for agents with no elevated permission, and is remembered per staff member per device in `localStorage`. It hides chrome (nav tabs, tray buttons, filter pills) and scopes queries (the day list, the strip dots, the bell), while every route keeps enforcing the real permission flags. No new RLS, no new role enum.

5. **A buffer between bookings is a phase.** `lng_booking_type_phases.is_buffer` marks a trailing, always-passive phase that holds the previous phase's pools (migration `20260915000002`). The admin sets it with one "Buffer after" control; every scheduler reads it as an ordinary phase. Whether a phase is a buffer is structural and read from the parent; children override only its length. Patient-facing surfaces (patient-facing duration, email timeline) skip it. A `buffer_minutes` column with checker-side arithmetic was rejected: it would have been a second representation of held time next to the phase model.
