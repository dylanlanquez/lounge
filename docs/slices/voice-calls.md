...
- **Clinic mode never shows voice calls** (Dylan, 15 Sep 2026): the day list, the week-strip dots, the free-time and down-time figures and the bell all exclude `voice_call` rows. Voice call mode is the only view of the agents' diary. Clinic staff can still book a call for a patient from New booking; it lands in the agents' diary.
