...
8. **Corrections, super admin only.** A withdrawal recorded by mistake is reversed, never deleted: a reason is required, the row is stamped with who reversed it and when, and the event lands in `lng_event_log`. Reversed in the open period, Right now goes back up by the amount. Reversed inside a period already closed by a signed count, Right now stays (it starts from what was physically counted) and that count's expected figure is restated by the amount, so its difference shrinks: on 7 Sep 2026 reversing "WHW supplies" took the count from £686.10 expected (£105.13 over) to £774.60 expected (£16.63 over). The reversal sheet shows a "What changes" table with the exact before and after figures. For a closed-period withdrawal it asks which is true: the cash was in the safe when it was counted (the count's expected is restated, Right now stays) or the cash was not in the count (a "put back" adjustment adds it to Right now and the count's figures stand). An already-reversed withdrawal can be put back later with a "Put back" pill (`lng_cash_put_back_withdrawal`, which also undoes the earlier restatement). Adjustments live in `lng_cash_adjustments`, show as green "+£ Put back into the safe" rows on the activity card, and appear in the CSV and PDF. A count signed by mistake can be voided the same way: it shows as Voided, Lounge re-anchors on the previous count, and everything since flows back into the open period. A voided count disappears from Past counts for everyone; the super admin sees it in a collapsed Archive card under Past counts, and can open it and "Restore this count" if it was voided by mistake. Both are `SECURITY DEFINER` functions gated on `auth_is_super_admin()`.
...
10. **Count rota and home-screen reminder.** Super admin only, on the Cash counts page: pick the weekdays a count is due and the safe holder responsible, and add cover (who, from, to) for days they are off. `lng_cash_count_due` derives whether a count is outstanding from the rota and the signed counts, so nothing is materialised or dismissable: the responsible person (and the super admin) sees a banner on the Schedule home screen from the start of the due day until a count is signed, and a missed day stays up marked overdue.
11. **Sealed envelopes, ready to bank.** Jade counts every Friday and seals the counted cash in a signed envelope that stays in the safe until it is banked or collected, sometimes several weeks later. The count sheet offers "Seal the counted cash in an envelope" (label on the envelope, same witness); "Seal cash" on the Right now card does the same later. Sealing moves nothing: Right now stays the safe total and reads "of which £X sealed in N envelopes, £Y loose". The "Ready to bank" card lists every sealed envelope (amount, label, sealed when and by whom, witness, the count it came from) with a running total, and "Bank or collect" takes the chosen envelopes out in one action: it records the bank-deposit withdrawal under the two-person rule and stamps the envelopes banked or collected, with a collapsible history of what went where. `lng_cash_seal_envelope` refuses to seal more than the loose cash; `lng_cash_bank_envelopes` is atomic.
12. **Archive, delete for good.** A voided count can be deleted permanently from the Archive (super admin); its figures are written to the event log first.
12. **Who sees the page.** The Cash counts page and its wallet icon are shown only to safe holders and to "Safe viewer" (Admin → Staff, currently Stephen Vazquez, read-only). Financials access no longer opens it; Reports → Cash drawer still shows the figure to finance viewers.
...
- `20260907000018_lng_cash_put_back.sql` (`lng_cash_adjustments`, put-back function, reverse gains `p_put_back`)
- `20260907000019_lng_cash_safe_position_v6.sql` (adjustments in the running balance and as lines)
- `20260907000023_lng_cash_sealed_envelopes.sql` (envelopes table, seal and bank functions)
- `20260907000022_lng_cash_count_due_v2.sql` (done-today and next-due on the reminder)
- `20260907000021_lng_cash_count_rota.sql` (rota and cover tables, `lng_cash_count_due`, `lng_cash_delete_voided_count`)
- `20260907000020_lng_cash_restore_and_close_at_zero.sql` (`lng_cash_restore_count` for a count voided by mistake; a put-back no longer double-counts when its withdrawal's count is voided; 3 July count restated to close at £0 with the banking on it; 7 Sept count restored)
